...
(function () {
    ...
    /* Theme / Dark Mode Persistence and Toggle
       This script runs on every page (website-protection.js is loaded site-wide).
       It reads/writes `localStorage['laura_theme']` with values: 'dark' | 'light' | 'system'.
       If 'system' or missing, it follows the OS preference (prefers-color-scheme).
    */
    (function themeInit() {
        const THEME_KEY = 'laura_theme';
        ...
        // Initialize theme early (before DOMContentLoaded if possible)
        try {
            const initial = readStoredTheme();
            // If DOM not ready yet, apply to documentElement classList directly
            if (document.readyState === 'loading') {
                // Try to apply quickly
                applyTheme(initial);
